...
export default function ViewVerses({
  ...
  return (
    <main className="flex flex-col w-full gap-4 mx-auto">
      <div className="flex items-center gap-3 sticky top-0 z-1 p-4 bg-white shadow">
        <Button
          variant="classic"
          color="gold"
      ...
      <section
        ...
        className="flex flex-col gap-1 items-center text-center w-full p-7"
      >
        ...
        <Image
          ...
          width={500}
          height={120}
          className="flex pt-10 h-12 sm:h-20 w-44 sm:w-80 flex-1 items-center justify-center text-black"
        />
      </section>
      <section aria-label="verses-content" className="flex-col gap-1 items-center w-full px-7">
        <div className="flex flex-col h-fit divide-y divide-gray-400 w-full space-y-8">
          {verses &&
            Object.entries(verses).map(([key, value], index) => {
              const latin = value.words.map((w) => w.transliteration.text).join(' ');
              return (
                <div key={key} className="flex-col w-full h-fit pb-8">
                  ...
                  <p className="font-bold text-xl sm:text-4xl text-end font-lpmqFont leading-12 sm:leading-24">
                    {value.text_uthmani}{' '}
                    <span className="text-yellow-700">{fmtArabic.format(value.verse_number)}</span>
                  </p>
                  <p className="font-light text-sm sm:text-lg mt-8 text-yellow-800">
                    {capitalizeFirst(latin)}
                  </p>
                  <p className="font-light text-sm sm:text-lg mt-2">
